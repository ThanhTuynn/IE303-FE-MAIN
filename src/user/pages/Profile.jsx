...
const PersonalInformation = () => {
  ...
  if (!user) {
    return <div className="flex justify-center items-center h-screen text-gray-600">User data could not be loaded.</div>;
  }
      ...
      <div className="flex flex-col md:flex-row bg-white rounded-lg shadow p-6 gap-6 max-w-5xl w-full">
          ...
          <ul className="mt-6 w-full space-y-3 text-left">
            ...
            <li>
              <button
                className="flex items-center text-gray-700 hover:text-red-600 w-full text-left hover:bg-gray-100 px-2 py-2 rounded"
                onClick={() => {
                  // TODO: Implement actual logout logic here (e.g., clear token, redirect to login)
                  alert('Đã đăng xuất!');
                  navigate('/login');
                }}
              >
        ...
        <div className="w-full md:w-2/3 border rounded-lg p-6 relative bg-white">
          ...
          <div className="space-y-3 text-base">
            {/* Display fetched user data */}
            <p><span className="font-semibold">Tên đăng nhập:</span> {user.username || 'N/A'}</p>
            <p><span className="font-semibold">Họ và tên:</span> {user.name || 'N/A'}</p>
            <p><span className="font-semibold">Ngày sinh:</span> {user.dateOfBirth ? new Date(user.dateOfBirth).toLocaleDateString() : 'N/A'}</p>
            <p><span className="font-semibold">Giới tính:</span> {user.gender || 'N/A'}</p>
            <p><span className="font-semibold">Email:</span> {user.email || 'N/A'}</p>
            <p><span className="font-semibold">Số điện thoại:</span> {user.phoneNumber || 'N/A'}</p>
            <p><span className="font-semibold">Địa chỉ:</span> {user.address || 'N/A'}</p>
          </div>
        </div>
      </div>
    </div>
